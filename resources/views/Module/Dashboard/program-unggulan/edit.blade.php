...
<div class="container mx-auto">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Program Unggulan</h1>
        ...
        <form action="{{ route('superadmin.program-unggulan.update', $programUnggulan) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div class="flex items-center space-x-4">
                    <div class="relative">
                        <div id="currentPhoto" class="{{ $programUnggulan->image ? '' : 'hidden' }}">
                            <img src="{{ asset($programUnggulan->image) }}" alt="Current Photo" class="w-32 h-32 rounded-lg object-cover border-2 border-gray-200">
                        </div>
                        <div id="initialPlaceholder" class="{{ $programUnggulan->image ? 'hidden' : '' }} w-32 h-32 rounded-lg bg-gray-200 flex items-center justify-center">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('title') border-red-500 @enderror"
                    id="title" type="text" name="title" value="{{ old('title', $programUnggulan->title) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror"
                    id="description" name="description" rows="4" required>{{ old('description', $programUnggulan->description) }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('order') border-red-500 @enderror"
                    id="order" type="number" name="order" value="{{ old('order', $programUnggulan->order) }}" min="0" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <div class="mt-2">
                    <label class="inline-flex items-center">
                        <input type="radio" name="is_active" value="1" {{ old('is_active', $programUnggulan->is_active) ? 'checked' : '' }} class="form-radio">
                        <span class="ml-2">Aktif</span>
                    </label>
                    <label class="inline-flex items-center ml-6">
                        <input type="radio" name="is_active" value="0" {{ old('is_active', $programUnggulan->is_active) ? '' : 'checked' }} class="form-radio">
